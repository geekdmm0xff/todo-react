...
export class TodoFooter extends Component {
  render() {
    ...
    return (
      <div>
        <label>
          当前：已做 {dones.length} 件，未做 {undones.length} 件
        </label>
        <p className="footerInfo">
          <a
            className={classNames({ cur: showType == "show-all" })}
            onClick={() => {
              this.handleTageClick("show-all");
            }}
          ...
          <a
            className={classNames({ cur: showType == "only-undone" })}
            onClick={() => {
              this.handleTageClick("only-undone");
            }}
            href="javascript:void(0)"
            style={{ margin: "0 10px" }}
          >
          ...
          <a
            className={classNames({ cur: showType == "only-done" })}
            onClick={() => {
              this.handleTageClick("only-done");
            }}
            href="javascript:void(0)"
          >
            查看完成
